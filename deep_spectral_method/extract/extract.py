...
def extract_features(
    ...
    #images = torch.from_numpy(images.transpose((2,0,1))[np.newaxis,...]).type(torch.float)
...
def iou(limits, limits_2):
    x_left = np.maximum(limits[0], limits_2[0])
    y_top = np.maximum(limits[1], limits_2[1])
    x_right = np.minimum(limits[2], limits_2[2])
    y_bottom = np.minimum(limits[3], limits_2[3])
    ...
    bb1_area = (limits[2] - limits[0]) * (limits[3] - limits[1])
    bb2_area = (limits_2[2] - limits_2[0]) * (limits_2[3] - limits_2[1])
    iou = inter / float(bb1_area + bb2_area - inter)
...
def _extract_crf_segmentations(
    ...
    image = image.transpose((2,-1, 1, 0))[...,0]
    max = np.max(np.max(image, axis = 0), axis = 0)
    image /= max
    image *= 255
    ...
    return segmap_crf, unary_potentials
