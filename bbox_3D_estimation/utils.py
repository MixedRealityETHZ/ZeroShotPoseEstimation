...
def get_data(dataset, random_downsample):
    ...
    if random_downsample:
        ...
        random_indices = np.ravel([random_indices, old_shape+random_indices, 2*old_shape+random_indices, 3*old_shape+random_indices])
        #print(random_indices)
    ...
    with open(intrinsics) as f:
        intr = f.readlines()
        K = np.array([[float(intr[0]), 0, float(intr[2])],
